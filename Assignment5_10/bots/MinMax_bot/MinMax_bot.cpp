...
public:
    ...
    virtual int minMaxAlgo(OthelloBoard &board, Turn turn, int d, Move move);
...
Move MyBot::play(const OthelloBoard &board)
{
    ...
    for(Move neighbour : neighbours)
    {
        ...
        int heuristic_val = minMaxAlgo(temp_board, this->turn, d, neighbour);
...
int MyBot::minMaxAlgo(OthelloBoard &board, Turn turn, int d, Move move)
{
    ...
    {
        OthelloBoard inBoard = OthelloBoard(board);
        inBoard.makeMove(turn, move);
        printf("Child %d ==> (%d, %d) :\t Heuristic Value: %d\n", branchNodes, move.x, move.y, heuristic_calc(inBoard, HNo));
        branchNodes++;
    }*/
    int best_val = 0;
    if (this->turn == turn)
    {
        ...
        for (Move move : nbrs_of_nbr)
        {
            ...
            best_val = min(best_val,minMaxAlgo(nbr_temp_board,other(turn),d-1,move));
        }
    }
    ...
    else
    {
        ...
        for (Move move : nbrs_of_nbr)
        {
            best_val = max(best_val, minMaxAlgo(nbr_temp_board,other(turn),d-1,move));
        }
    }
    return best_val;
}
